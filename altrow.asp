...
<body>
  <div id="main">
    ...
    function print(result){
      Response.write("<form action=\"successaltrow.asp\" method=\"POST\"><table><caption>Confirm the line to be deleted</caption>" +           
        "<tr><td></td><th>current</th><th>new</th></tr><tr><th>" + result.fields(0).name + "</th><td>" + result.fields(0) + "</td><td></td></tr>");
      var numCols = result.fields.count;
      for (var col = 1; col < numCols; col++) {
        Response.write("<tr><th>" + result.fields(col).name + "</th><td>" + result.fields(col) + "</td>" + 
          "<td><input type=\"text\" name=\"" + result.fields(col).name + "\" value=\"\" required></td></tr>");
      }  
      Response.write("<input type=\"hidden\" name=\"key\" value=\"" + key + "\">" + 
        "<input type=\"hidden\" name=\"table\" value=\"" + table + "\">" + 
        "<input class=\"button\" type=\"reset\" value=\"reset\"><input class=\"button\" type=\"submit\" value=\"confirm\"></table></form>");
    }  
    function print1(){
