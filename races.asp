...
<body>
  <div id="main">
    ...
    <nav>
      <div class="table">
        <div class="row">
          <div class="cell">
            <form name="home" method="POST" action="index.asp">
              <%
                    print();
                  %>
              <a class="mailtable" href="javascript: document.home.submit();"><img id="home" src="assets/img/button_home.jpg" alt="home"></a>
            </form>
          </div>
          <div class="cell">
          ...
          <div class="cell">
            <a class="mailtable" href="javascript:void(0);">Races</a>
          </div>
          <div class="cell">
    ...
    <section id="sectionID">
      <form id="formID" method="POST" action="races.asp">
        <%
      ...
      if (flag == "-1"){
        ...
        Response.write(str + id + "<a href=\"races.asp\">(exit)</a><input type=\"hidden\" name=\"flag\" value=\"1\">");
      }
      else if (flag == "2"){
        Response.write(str + id + "<a href=\"races.asp\">(exit)</a><input type=\"hidden\" name=\"flag\" value=\"2\">"); 
      }
    %>
      </form>
    </section>
    <section class="mainsection">
      <table>
        ...
        if (flag == "1" || flag == "2"){
          try{   
            printTable(conn.Execute("select * from race"));
          }
          catch(exception){
          ...
          function printTable(results) 
          {
            ...
            Response.write("<table class=\"datatable\"><caption>Race table</caption><tr>");
          ...
          if (flag == "2"){
            Response.write("<div><form id=\"addform\" action=\"addrow.asp\" method=\"POST\"><table><caption>Add new race</caption>" + 
            "<tr><th>Race Name</th><td><input type=\"text\" name=\"RaceName\" value=\"\" required></td></tr>" +
            "<tr><th>Address</th><td><input type=\"text\" name=\"Address\" value=\"\" required></td></tr>" +
            "<tr><th>Length</th><td><input type=\"text\" name=\"Length\" value=\"\" required></td></tr>" +
            "</table><input type=\"hidden\" name=\"table\" value=\"race\"><input class=\"button\" type=\"reset\" value=\"reset\"><input class=\"button\" type=\"submit\" value=\"confirm\"></form></div>" + 
            "<div><h2>Select the line to be deleted or update</h2><select id=\"Select\">");
            try{
              result = conn.Execute("select key from race");
              while(!result.eof){
            ...
            "<input id=\"del\" type=\"hidden\" name=\"del\" value=\"\"><input type=\"hidden\" name=\"table\" value=\"race\">" + 
            "<input id=\"delButton\" class=\"button\" type=\"submit\" value=\"delete\"></form>" + 
            "<form  id=\"altrow\" action=\"altrow.asp\" method=\"POST\">" + 
            "<input id=\"alt\" type=\"hidden\" name=\"alt\" value=\"\"><input type=\"hidden\" name=\"table\" value=\"race\">" + 
