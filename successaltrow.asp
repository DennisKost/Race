  ...
  <div id="main">
    ...
    try{
      if (table == "race"){      
        ...
        conn.Execute("update race set RaceName = \'" + RaceName + "\', Address = \'" + Address +  "\', Length = " + Length + " where key = " + key);      
        Response.write("<p>Record successfully updated!</p>");  
      }
      else if (table == "team"){
        ...
        if (!conn.Execute("select 1 from sponsor where key = " + Sponsor).eof){
          conn.Execute("update team set TeamName = \'" + TeamName + "\', Country = \'" + Country +  "\', Budget = \'" + Budget + "\'," +
            "FDate = \'" + FDate + "\', Address = \'" + Address + "\', Sponsor = \'" + Sponsor + "\' where key = " + key);
          Response.write("<p>Record successfully updated!</p>");
        }
        else{
        ...
        if (!conn.Execute("select 1 from team where key = " + Team).eof){
          conn.Execute("update racer set RacerName = \'" + RacerName + "\', Age = " + Age +  ", Address = \'" + Address + "\'," +
            "Team = " + Team + " where key = " + key); 
          Response.write("<p>Record successfully updated!</p>");
        }
        else{
        ...
        conn.Execute("update sponsor set SponsorName = \'" + SponsorName + "\', Country = \'" + Country +  "\', Field = \'" + Field + "\' where key = " + key); 
        Response.write("<p>Record successfully updated!</p>");      
      }
        ...
        if (flagTeam && flagRace){
          ...
          conn.Execute("update participation set Team = " + Team + ", Race = " + Race + " where key = " + key);        
          Response.write("<p>Record successfully updated!</p>");
        }        
      }
      else if (table == "prize"){
        ...
        if (flagRace && flagRacer){
          ...
          conn.Execute("update prize set Race = " + Race + ", Racer = " + Racer + ", Cash = \'" + Cash + "\' Place = " + Place + " where key = " + key);
          Response.write("<p>Record successfully updated!</p>");
        }
      }      
    }
